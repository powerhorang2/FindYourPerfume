...
<script type="text/javascript">
	...
	function createCmtUpd(item) {
	   ...
	   cmt_upd_bt.addEventListener('click', event => cmtUpdExe(item));
	...
	function CmtUpdReturnChk(item) {
	   ...
	   if (confirm("댓글 수정을 취소 하시겠습니까??") == true) {    //확인
		  ...
		  ajaxSelPageCnt(perfume.i_p) // 요주인물
	...
	function changeCmtUpdSuccess(item) {
		...
		cmt_suc_bt.removeEventListener('click', cmtUpdExe); // 작동을 안함 이거 해결 필요 
	...
	function changeCmtUpdReturn(item) {
	   ...
	   cmt_ret_bt.removeEventListener('click', cmtDel); // 작동을 안함 이거 해결 필요 
